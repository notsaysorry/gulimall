...
import com.atguigu.gulimall.seckill.config.LoginUserInterceptor;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    @Override
    public SeckillSkuRedisTo getSkuSeckilInfo(String skuId) {
...
//            String reg = "\\d-" + skuId;
    ...
    private void saveSessionInfos(List<SeckillSessionWithSkusVo> sessions) {
        ...
        sessions.stream().forEach(session -> {
            ...
            //获取当前活动的开始和结束时间的时间戳
            long startTime = session.getStartTime().getTime() - 8 * 3600 * 1000;
            long endTime = session.getEndTime().getTime() - 8 * 3600 * 1000;
        ...
        sessions.stream().forEach(session -> {
            ...
            session.getRelationSkus().stream().forEach(seckillSkuVo -> {
                ...
                if (!operations.hasKey(redisKey)) {
                    ...
                    String skuId = seckillSkuVo.getSkuId();
                    ...
                    //3、设置当前商品的秒杀时间信息
                    redisTo.setStartTime(session.getStartTime().getTime() - 8 * 3600 * 1000);
                    redisTo.setEndTime(session.getEndTime().getTime() - 8 * 3600 * 1000);
                    ...
                    operations.put(seckillSkuVo.getPromotionSessionId() + "-" + seckillSkuVo.getSkuId(), seckillValue);
